...
const emptyProfile = { display_name: '', short_description: '', bio: '', hobbies: '', interests: '', profile_image_url: '', is_published: false, consent_confirmed: false }
...
export default function AdminWorkspace() {
  ...
  const editProfile = (profile?: Profile) => {
    ...
    setForm(profile ? { display_name: profile.display_name, short_description: profile.short_description, bio: profile.bio, hobbies: profile.hobbies.join(', '), interests: profile.interests.join(', '), profile_image_url: profile.profile_image_url, is_published: profile.is_published, consent_confirmed: profile.consent_confirmed } : emptyProfile)
    setImage(undefined)
  }
  const saveProfile = async (event: React.FormEvent) => {
    event.preventDefault(); if (!supabase) return
    let imageUrl = form.profile_image_url
    if (image) { const path = `${crypto.randomUUID()}-${image.name}`; const upload = await supabase.storage.from('profile-images').upload(path, image); if (upload.error) { setMessage(upload.error.message); return } imageUrl = supabase.storage.from('profile-images').getPublicUrl(path).data.publicUrl }
    const payload = { display_name: form.display_name, short_description: form.short_description, bio: form.bio, hobbies: form.hobbies.split(',').map(value => value.trim()).filter(Boolean), interests: form.interests.split(',').map(value => value.trim()).filter(Boolean), profile_image_url: imageUrl, is_published: form.is_published, consent_confirmed: form.consent_confirmed }
